...
export default function TasteDetailTest() {
  ...
  const { detailCode, details } = useMemo(() => {
    ...
    DIM_ORDER.forEach((dim) => {
      const b = buckets[dim];
      const avg = b.length ? +(b.reduce((a, v) => a + v, 0) / b.length).toFixed(2) : 3.0;
  ...
  const goBackToResult = () => {
    if (baseType) {
      navigate('/result', { state: { mbtiType: baseType, answers: baseAnswers } });
    } else {
      navigate('/');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-background text-foreground px-4 py-10">
      <div className="max-w-2xl mx-auto p-6 bg-card text-card-foreground rounded-lg border border-border shadow-card">
        {!done ? (
          <>
            <h1 className="text-2xl font-medium mb-3">🔎 입맛 MBTI 세부 검사</h1>
          ...
          <>
            ...
            <p className="text-muted-foreground mb-6">
              {baseType ? (
                <>메인 유형 <span className="font-medium text-foreground">{baseType}</span> 기준으로 서브 지표를 보여드려요.</>
              ) : (
                '서브 지표를 확인해보세요.'
              )}
            </p>
            ...
            <ul className="divide-y divide-border">
              {DIM_ORDER.map((dim) => {
                ...
                return (
                  <li key={dim} className="py-4 flex items-start justify-between gap-4">
                    <div>
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-xs text-muted-foreground mt-0.5">{d.rank}</div>
                    </div>
                  </li>
                );
              })}
